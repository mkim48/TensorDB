#!/usr/bin/python
#
# comp_cov_comp.py computes the covariance matrices for each mode of an input tensor in dense representation using the compressed matrix multiplication (multiply_comp operator) for Dynamic Tensor Decomposition (DTA: Dynamic Tensor Analysis), which is used to incrementally update the tensor decomposition.
#
# Note 
#  - 'comp_cov_comp.py' generates approximate covariance matrices 
#
# Parameters
...
#  - old_tensor_name (optional): an old tensor on which the new tensor is incrementally updated.
...
def main():
	argcnt = len(sys.argv)
	if argcnt < 5:
		print "comp_cov_comp.py <tensor_name> <tensor_size> <chunk_size> <threshold> <old_tensor_name> <debug>"
		exit(2)
	...
	for i in range(nmode):	
		...
		if debug == 0:
			...
			print query
		if int(tensz[i]) > threshold:
			query="store(repart("+mat+str(i)+",<val:double>[i=1:"+str(tensz[i])+","+str(tensz[i])+",0,j=1:"+str(prod_sz)+","+str(prod_chunksz)+",0]),"+mat+"_repart"+str(i)+")"
			if debug == 0:
				print query
				f.write(query+'\n')
				result=db.executeQuery(query,"afl")
				...
				print query
			query="multiply_comp(transpose("+mat+"_repart"+str(i)+"),transpose("+mat+"_repart"+str(i)+"))"
		else:
