#!/usr/bin/python
#
# dta_withcov.py runs Dynamic Tensor Decomposition (DTA: Dynamic Tensor Analysis). Given the input tensor, incrementally update the tensor decomposition. This operation requires the covariance matrices of the input tensor, which are computed by comp_cov_dense.py, comp_cov_sparse.py, or comp_cov_comp.py. 
#
# Parameters
...
#
# Examples
#  dta_withcov.py tensor1 1000,1000,1000 100,100,100 2,3,4
#
#   - The example takes a 3-mode tensor of size 1000x1000x1000 with the chunk size 100x100x100 and decompose the tensor. The output of this example includes factor matrices (tensor1_fac_0, tensor1_fac_1, and tensor1_fac_2) and a core (tensor1_core).
#
# BEGIN_COPYRIGHT
#
...
def main():
	argcnt = len(sys.argv)
	if argcnt < 5:
		print "dta_withcov.py <tensor_name> <tensor_size> <chunk_size> <rank> <debug>"
		exit(2)
	...
	if debug == 0:
		msg = 'dta_withcov Time elapsed: %d min %d.%d sec.' % (minutes, seconds, microsec)
		f.write(msg+'\n\n')
		f.close()
