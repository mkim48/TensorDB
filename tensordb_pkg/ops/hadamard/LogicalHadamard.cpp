...
/**
 * @brief The operator: hadamard().
 *
 * @par Synopsis:
 *   hadamard( leftArray, rightArray )
 *
 * @par Summary:
 *   Hadamard product
 *   This operator is created based on multiply(). The error codes for multiply() are shared.
 */

class LogicalHadamard : public  LogicalOperator
{
public:
	LogicalHadamard(const std::string& logicalName, const std::string& alias):
    ...
    ArrayDesc inferSchema(std::vector< ArrayDesc> schemas, boost::shared_ptr< Query> query)
    {
		...
		AttributeDesc multAttr((AttributeID)0, "hadamard", type, 0, 0);
		atts[0] = multAttr;
        ...
        return ArrayDesc("Hadamard",atts,dims);
	}

};

DECLARE_LOGICAL_OPERATOR_FACTORY(LogicalHadamard, "hadamard")
