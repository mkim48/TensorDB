...
class PhysicalMaxNormCoreArray : public  PhysicalOperator
{
public:
    PhysicalMaxNormCoreArray(std::string const& logicalName,
            ...
            boost::shared_ptr<Query> query)
	{
		...
		while (!iter->end()) {
			...
			ConstChunk const* chunk = &iter->getChunk();
		 	PinBuffer s1(*chunk);
			double* val = (double*)chunk->getData();

            if (chunk->isRLE()) {
                MemChunk leftMatChunk;
                ConstRLEPayload leftPayload((char const*)val);
                if (leftPayload.nSegments() != 1 || leftPayload.getSegment(0).same || leftPayload.getSegment(0).length() != iChunkLen*jChunkLen) {
                    ...
                    val = (double*)leftMatChunk.getData();
                } else {
                    val = (double*)leftPayload.getRawValue(leftPayload.getSegment(0).valueIndex);
                }
            }
			...
			for(Coordinate i=start[0];i<=last[0];i++) {
				for(Coordinate j=start[1];j<=last[1];j++) {
					if(val[(i-start[0])*jChunkLen+(j-start[1])] > aggvals[i-start[0]]) // max_norm  
					{
						aggvals[i-start[0]] = val[(i-start[0])*jChunkLen+(j-start[1])];
					}
				}
			}	
			++(*iter);
...
DECLARE_PHYSICAL_OPERATOR_FACTORY(PhysicalMaxNormCoreArray, "maxNormCoreArray", "PhysicalMaxNormCoreArray")
