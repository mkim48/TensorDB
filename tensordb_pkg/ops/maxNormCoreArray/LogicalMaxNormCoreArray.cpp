...
/**
 * @brief The operator: maxNormCoreArray().
 *
 * @par Synopsis:
 *   maxNormCoreArray( array, core )
 *
 * @par Summary:
 *   Normalize array by max-norm and the weight is copied into core
 *
 */
class LogicalMaxNormCoreArray : public  LogicalOperator
{
public:
	LogicalMaxNormCoreArray(const std::string& logicalName, const std::string& alias):
...
DECLARE_LOGICAL_OPERATOR_FACTORY(LogicalMaxNormCoreArray, "maxNormCoreArray")
