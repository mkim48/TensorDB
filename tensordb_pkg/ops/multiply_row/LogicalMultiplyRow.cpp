...
/**
 * @brief The operator: multiply_row().
 *
 * @par Synopsis:
 *   multiply_row( leftArray, rightArray )
 *
 * @par Summary:
 *   Matrix multiplication of entries of rows of each matrix so that both matrices can take advantage of the row-wise data ordering.
 *   The two matrices must have the same size of 'row' dimension and same chunk size along that dimension.
 *   This operator is created based on multiply(). The error codes of multiply() are used.
 *
 */
class LogicalMultiplyRow : public  LogicalOperator
{
public:
	LogicalMultiplyRow(const std::string& logicalName, const std::string& alias):
...
DECLARE_LOGICAL_OPERATOR_FACTORY(LogicalMultiplyRow, "multiply_row")
