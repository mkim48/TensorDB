...
/**
 * @brief The operator: khatrirao().
 *
 * @par Synopsis:
 *   khatrirao( leftArray, rightArray )
 *
 * @par Summary:
 *   Khatri-Rao product 
 *
 */
class LogicalKhatriRao : public  LogicalOperator
{
public:
	LogicalKhatriRao(const std::string& logicalName, const std::string& alias):
    ...
    ArrayDesc inferSchema(std::vector< ArrayDesc> schemas, boost::shared_ptr< Query> query)
    {
		...
		AttributeDesc multAttr((AttributeID)0, "khatrirao", type, 0, 0);
		atts[0] = multAttr;

		Dimensions dims(2);
		DimensionDesc const& d1 = schemas[0].getDimensions()[1];
		DimensionDesc const& d2 = schemas[0].getDimensions()[0];
		DimensionDesc const& d3 = schemas[1].getDimensions()[1];
                dims[1] = DimensionDesc(d1.getBaseName(),
                                1,
                                d1.getLength()*d3.getLength(),
                                d1.getChunkInterval()*d3.getChunkInterval(),
                                0,
                                d1.getType(),
                                d1.getFlags(),
		...
		dims[0] = DimensionDesc(d1.getBaseName() == d2.getBaseName() ? d1.getBaseName() + "2" : d2.getBaseName(), 
        ...
        return ArrayDesc("KhatriRao",atts,dims);
	}

};

DECLARE_LOGICAL_OPERATOR_FACTORY(LogicalKhatriRao, "khatrirao")
